                        ...
                        if (flat_cross) {
                            translate([0,0,0])
                                ...
                                if (stem_type == "cherry_cross") {
                                                                rotate([0,0,45])
cherry_cross(cross_height, y_adjust=cross_y_extra, x_adjust=cross_x_extra);
                                } else if (stem_type == "fused_cross") {
                                                                        
                                    rotate([0,0,135]) fused_cross(cross_height, y_adjust=cross_y_extra, x_adjust=cross_x_extra);
                                }
                        }
...
// This makes an x shape like the Cherry cross above, but with the bottom legs fused together, since that's how it ends up on some printers anyway.
module fused_cross(height=CHERRY_CROSS_HEIGHT, y_adjust=0, x_adjust=0) {
    linear_extrude(height=height) {
        difference() {
            union() {
               square([CHERRY_CROSS_THICKNESS_X+x_adjust, CHERRY_CROSS_LENGTH], center=true);
        ...
        translate([-CHERRY_CROSS_LENGTH/2,-(CHERRY_CROSS_THICKNESS_X+x_adjust)/2,0]) {
            rotate([0,0,45]) {
...
stem_cherry_cross(2, 4, 4, 2,1, "fused_cross");